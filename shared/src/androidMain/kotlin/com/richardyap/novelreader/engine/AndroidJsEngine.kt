...
/**
 * Android actual JsEngine 实现 —— 使用 Mozilla Rhino。
 */
actual class JsEngine actual constructor() {
    ...
    actual fun search(keyword: String): List<OnlineBookItem> {
        val ctx = rhinoContext ?: return emptyList()
    ...
    actual fun bookInfo(bookUrl: String): Pair<Book, List<OnlineChapterInfo>> {
        val ctx = rhinoContext ?: return Book(
    ...
    private fun injectLegadoApi() {
        ...
        // legado.getDom(u, headers) -> 返回一个提供 DOM 操作的包装对象
        val getDom = object : org.mozilla.javascript.BaseFunction() {
            override fun call(cx: Context, scope: Scriptable, thisObj: Scriptable, args: Array<out Any?>): Any {
                ...
                // dom.select(cssQuery)
                (domObj as ScriptableObject).defineFunctionProperties( arrayOf("select", "selectFirst", "attr", "text", "html"), domObj.javaClass,
                    ScriptableObject.DONTENUM
                )

                domObj.put("_doc", domObj, docRef)
        ...
        (legadoObj as ScriptableObject).defineFunctionProperties( 
            arrayOf("getHttp", "getDom", "postHttp", "encodeUrl", "resolveUrl"),
            legadoObj.javaClass,
            ScriptableObject.DONTENUM,
        )
    ...
    private fun parseOnlineBookItem(obj: NativeObject): OnlineBookItem {
        return OnlineBookItem(
            name = obj.get("name", obj)?.toString() ?: obj.get("title", obj)?.toString() ?: "",
            bookUrl = obj.get("bookUrl", obj)?.toString() ?: obj.get("url", obj)?.toString() ?: "",
            author = obj.get("author", obj)?.toString() ?: "",
            coverUrl = obj.get("coverUrl", obj)?.toString() ?: obj.get("cover", obj)?.toString() ?: "",
            tocUrl = obj.get("tocUrl", obj)?.toString() ?: obj.get("chapterUrl", obj)?.toString() ?: "",
            intro = obj.get("intro", obj)?.toString() ?: obj.get("description", obj)?.toString() ?: "",
            latestChapter = obj.get("latestChapter", obj)?.toString() ?: obj.get("lastChapter", obj)?.toString() ?: "",
            ...
            kind = obj.get("kind", obj)?.toString() ?: obj.get("type", obj)?.toString() ?: "",
        )
    }
